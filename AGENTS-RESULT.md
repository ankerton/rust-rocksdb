...
## Summary
Applied the one-line fix in `librocksdb-sys/build.rs` to exclude `db/c.cc` from `lib_sources` when the `encrypted-env` feature is active. This prevents the duplicate symbol linker error caused by `crocksdb/crocksdb.cc` re-exporting the same C API symbols that `rocksdb/db/c.cc` defines.
